# CTfind Production Control - Phase 1

## 1. Назначение

Phase 1 покрывает базовую цифровизацию заказов и производственных задач.

## 2. Майлстоуны Phase 1

### Milestone 1. Основа системы

- авторизация
- bootstrap суперадмина
- пользователи
- редактирование пользователей
- роли
- права доступа
- справочник сотрудников

### Milestone 2. Контур заказов

- создание заказа
- редактирование заказа
- список заказов
- поиск и фильтрация
- статусы заказа:
  - новый
  - в работе
  - готов
  - отгружен
- карточка заказа

### Milestone 3. Контур производственных задач

- создание задач внутри заказа
- назначение исполнителя
- установка срока
- статусы задачи:
  - не начато
  - в работе
  - выполнено
- связь заказа и задач

### Milestone 4. Рабочие интерфейсы

- доска задач мастера
- список задач сотрудника
- отображение статусов
- базовая навигация между заказом и задачами

### Milestone 5. Контроль исполнения

- просрочки задач
- контроль сроков
- фильтры по статусам
- фильтры по исполнителям
- фильтры по срокам

### Milestone 6. Прозрачность изменений

- журнал действий
- история изменений заказа
- история изменений статусов
- фильтрация журнала

### Milestone 7. Внутренние уведомления

- новое назначение
- изменение статуса
- просрочка задачи

Канал: только внутри системы.

## 3. Пользовательские роли

### Администратор (`ADMIN`)

- управление пользователями
- управление ролями
- доступ ко всем разделам
- просмотр журнала действий

### Менеджер заказов (`ORDER_MANAGER`)

- создание заказов
- редактирование заказов
- просмотр статусов
- контроль сроков заказа

### Мастер / начальник цеха (`PRODUCTION_SUPERVISOR`)

- создание задач по заказу
- назначение исполнителей
- просмотр доски задач
- контроль просрочек

### Исполнитель (`PRODUCTION_EXECUTOR`)

- просмотр своих задач
- перевод задачи в работу
- перевод задачи в выполнено

### Склад (`WAREHOUSE`)

- просмотр материалов
- просмотр остатков
- списание под заказ

## 4. Основные экраны

- список заказов
- карточка заказа
- доска задач
- список задач сотрудника
- склад
- журнал действий
- пользователи (только ADMIN)

## 5. Ограничения Phase 1

В Phase 1 не входят:

- планирование смен
- планирование мощностей
- расчет загрузки ресурсов
- управление оборудованием
- MRP
- финансы
- бухгалтерия
- сложные интеграции
- AI-функции
- Telegram/email уведомления

## 6. Упрощения в рамках Phase 1

### Загрузка

Отображается только как:

- количество задач на исполнителе
- количество задач на этапе
- количество просроченных задач

### Склад

Учитываются только:

- материалы
- остатки
- списание под заказ

Без партий, резервирования и сложного складского учета.

## 7. Компонентная схема

```mermaid
flowchart TB
    subgraph U["Пользователи"]
        A["Администратор"]
        M["Менеджер заказов"]
        C["Мастер / начальник цеха"]
        E["Исполнитель"]
        W["Склад"]
    end

    subgraph UI["Интерфейсы"]
        UI1["Список заказов"]
        UI2["Карточка заказа"]
        UI3["Доска задач"]
        UI4["Список задач сотрудника"]
        UI5["Склад"]
        UI6["Журнал действий"]
    end

    subgraph CORE["Компоненты Phase 1"]
        AUTH["Авторизация и RBAC"]
        ORD["Управление заказами"]
        TASK["Управление задачами"]
        STAFF["Пользователи и роли"]
        NOTIF["Внутренние уведомления"]
        AUDIT["Журнал действий"]
        INV["Упрощенный склад"]
    end

    subgraph DATA["Данные"]
        D1[("Пользователи")]
        D2[("Заказы")]
        D3[("Задачи")]
        D4[("Материалы и остатки")]
        D5[("События аудита")]
        D6[("Уведомления")]
    end

    A --> UI1
    A --> UI2
    A --> UI3
    A --> UI5
    A --> UI6

    M --> UI1
    M --> UI2

    C --> UI2
    C --> UI3

    E --> UI4

    W --> UI5

    UI1 --> AUTH
    UI2 --> AUTH
    UI3 --> AUTH
    UI4 --> AUTH
    UI5 --> AUTH
    UI6 --> AUTH

    AUTH --> STAFF
    UI1 --> ORD
    UI2 --> ORD
    UI2 --> TASK
    UI3 --> TASK
    UI4 --> TASK
    UI5 --> INV
    UI6 --> AUDIT

    ORD --> D2
    TASK --> D3
    STAFF --> D1
    INV --> D4
    AUDIT --> D5
    NOTIF --> D6

    ORD --> AUDIT
    TASK --> AUDIT
    STAFF --> AUDIT
    INV --> AUDIT

    ORD --> NOTIF
    TASK --> NOTIF

    ORD --> TASK
    TASK --> STAFF
    INV --> ORD
```



## 8. Критерии завершения

Phase 1 завершена, если система позволяет:

- обеспечить первичный вход администратора (локально и production-like bootstrap)
- создавать пользователей, редактировать их имена и роли, назначать им только канонические role-коды Phase 1
- создать заказ
- изменить заказ
- просматривать список и карточку заказа с текущим статусом
- перевести заказ в работу
- разбить заказ на задачи
- назначить исполнителей
- открывать доску производственных задач для мастера и рабочий список исполнителя с учетом ролевой видимости
- менять статусы задач
- видеть просрочки
- видеть текущий статус заказа
- видеть текущий статус задач
- просматривать журнал действий
- видеть внутренние уведомления о назначениях, изменениях статусов и просрочках внутри системы
- вести упрощенный складской учет материалов, приходов и текущих остатков
- списывать материалы под заказ и видеть расход материалов в контексте заказа

На момент выравнивания Phase 1 от 2026-05-03 продуктовые возможности из списка выше не выносятся из scope Phase 1. Отложенные ручные проверки и smoke/sign-off items фиксируются отдельно в `specs/016-phase1-alignment/quickstart.md` и не меняют функциональный scope.
